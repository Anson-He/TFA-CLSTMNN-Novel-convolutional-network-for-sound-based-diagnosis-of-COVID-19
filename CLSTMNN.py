...
model.add(layers.Conv1D(8, 3, activation='relu', kernel_regularizer=regularizers.l2(0.01)))
model.add(layers.Dropout(0.2))
model.add(layers.MaxPooling1D(pool_size=(3)))
model.add(layers.Dense(64))
model.add(layers.Dropout(0.2))
model.add(layers.BatchNormalization())
model.add(layers.LSTM(16, activation='tanh', kernel_regularizer=regularizers.l2(0.01), return_sequences=True))
model.add(layers.Dropout(0.2))
...
plot_model(model, to_file='./CLSTMNN/CLSTMNN_model.png', show_shapes=True)
model.summary()
...
callbacks_list=[
    ...
    tensorflow.keras.callbacks.ModelCheckpoint(
        filepath='./CLSTMNN/CLSTMNN_checkpoint.h5',
        monitor='val_loss',
        save_best_only=True
    ),
    tensorflow.keras.callbacks.TensorBoard(
        log_dir='./CLSTMNN/CLSTMNN_train_log'
    )
]
...
model.save('./CLSTMNN/CLSTMNN_model.h5')
model.save_weights('./CLSTMNN/CLSTMNN_weight.h5')
